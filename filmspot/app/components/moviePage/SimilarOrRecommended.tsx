...
function SimilarOrRecommended({props}: {props: Props}){
    ...
    const more = moreContent?.map((item: MovieInfo) => {
        const banner = item.poster_path?.length > 0 ? `${import.meta.env.VITE_TMDB_POSTER_BASE_URL}/${item.poster_path}` : NoBanner;
        const genres = item.genre_ids.filter(id => id in tmdbAllGenres).slice(0, 3).map(id => tmdbAllGenres[id]);

        return (
            <div key={item.id} className="contentItem snap-center">
                <span className="topShadow pointer-events-none"></span>
                <img className="pointer-events-none" src={banner} alt="Banner" />
                ...
                <Link to={`/content/${props.type}.${item.id}`} className="absolute w-full h-full top-0 left-0 z-1"></Link>

                <button name={`movie${item.id}`} className={`bookmark absolute button z-2 ${isInWishlist(item.id) ? "open" : ''}`} onClick={() => saveToWatchlist(item.id, undefined, {banner: banner, name: item.title ?? item.name, year: item.release_date ? item.release_date?.split('-')[0] : item.first_air_date?.split('-')[0], genres: genres, description: item.overview, type: props.type})} >
