function ButtonWithArrow({title, additionalClasses, onClick}: {title: string, additionalClasses?: string, onClick?: () => void}){
    return (
        <button className={`button castButton ${additionalClasses ?? ''}`} onClick={onClick}>
            {title}
