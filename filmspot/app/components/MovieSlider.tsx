...
function MovieSlider({props}: {props: {content: any, scrollValue?: number}}){
    ...
    return <>
        <span className="relative block">
            <span className="content fade-mask snap-x snap-mandatory" ref={scrollRef} onScroll={() => {handleMovieSliderScroll(scrollRef); disableButtons.current = true;}} onScrollEnd={() => {disableButtons.current = false}} >
                {props.content}
            </span>
